...
// *********************************************************************************
if (!function_exists('fetchType')) {
    function fetchType($id)
    {
        $data = Type::where('is_active', 1)->get();
        // dd($data);
        foreach ($data as $key => $val) {
            if ($id == $val->id) {
                echo "<option value='" . $val->id . "' selected>" . $val->name . "</option>";
...
// *********************************************************************************
function getItemOptions($selectedItem)
{
    ...
    foreach ($items as $item) {
        $items = Item::all();
        $options = '';
        $selectedItemArray = ($selectedItem instanceof Collection) ? $selectedItem->toArray() : $selectedItem;
        foreach ($items as $item) {
            $itemId = $item->id;
            $selected = in_array($itemId, $selectedItemArray) ? 'selected' : '';
...
{
    // Generate a unique order number logic goes here
    return 'ORD' . uniqid();
}
// *********************************************************************************
// *********************************************************************************
